...
inline string serialize(const LWWPairLattice<string>& l) {
  ...
  lww_value.set_promise(l.reveal().promise);
...
inline string serialize(const MultiKeyWrenValue &l) {
    MultiKeyWrenValue multiKeyWrenValue;
    multiKeyWrenValue.set_committime(l.committime());
    multiKeyWrenValue.set_value(l.value());

    string serialized;
    multiKeyWrenValue.SerializeToString(&serialized);
    return serialized;
}
...
inline LWWPairLattice<string> deserialize_lww(const string& serialized) {
  ...
  return LWWPairLattice<string>(
      TimestampValuePair<string>(lww.timestamp(), lww.promise(), lww.value()));
}
...
inline MultiKeyWrenValue deserialize_multi_key_wren(
        const string& serialized) {
    MultiKeyWrenValue mk_causal;
    mk_causal.ParseFromString(serialized);
...
inline TimestampValuePair<string> to_multi_key_wren_payload(
        const MultiKeyWrenValue& mkcv) {
    return TimestampValuePair<string>(mkcv.committime(), mkcv.value());
}
